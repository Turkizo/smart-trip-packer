  ...
  return (
    <li className="flex items-center py-3">
      <label className="flex items-center cursor-pointer group w-full">
        ...
        />
        <div
          className={`w-6 h-6 flex-shrink-0 border-2 rounded-md flex items-center justify-center ml-3 transition-all duration-200 ${
            item.packed
        ...
        >
          <span className="ml-2 text-amber-300" title="הצעה מ-AI">{item.source === 'ai' ? '✨' : ''}</span>
          {item.name}
        </span>
      </label>
    </li>
